    ...
    out float4 oPos : OUTPOSITION)
{
    ...
    oReflectUV.zw = iTexCoord * cNoiseTiling + cElapsedTime * cNoiseSpeed;
    oNormal = GetWorldNormal(modelMatrix);
    oEyeVec = float4(cCameraPos - worldPos, GetDepth(oPos));
    oTexCoord = GetTexCoord(iTexCoord) * cDiffTiling + cElapsedTime * cNoiseSpeed;
    ...
    out float4 oColor : OUTCOLOR0)
{
    ...
    float2 noise = (Sample2D(NormalMap, iReflectUV.zw).rg - 0.5) * cNoiseStrength;
    refractUV += noise;
    ...
    float3 finalColor = diffColor.rgb *diffColor.a + lerp(refractColor, reflectColor, fresnel);
